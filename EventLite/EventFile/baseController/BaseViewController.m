...
- (void) createBarWithName:(NSString *)title
                     ...
                     action:(SEL)action
{
    ...
    switch (type) {
        case NavBarType_left:
            self.navigationItem.leftBarButtonItem = barButton;
            break;
        case NavBarType_right:
...
- (UIBarButtonItem *) barButtonItemWithBarItemType:(MoshNavigationBarItemType)type isLeft:(BOOL)isLeft
{
    switch (type) {
        ...
        case MoshNavigationBarItemCheck:
            ...
            break;
        case MoshNavigationBarItemPrint:
            return [BaseViewController createNavigationBarItemWithNormalImageName:NAVBUTTON_print highlightImageName:NAVBUTTON_print_h title:nil isLeft:isLeft action:@selector(navPrintClick)target:self];
            break;
...
- (void) showAlertViewWithTitle:(NSString *)title
              ...
              CancleButtonTitle:(NSString *)cancle
                    selectBlock:(AlertViewBlock)block
              otherButtonTItles:(NSString *)otherButtonTitles, ...
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:message delegate:self cancelButtonTitle:cancle otherButtonTitles:otherButtonTitles, nil];
...
- (NSString *) getDatabaseTicketID:(NSArray *)array
{
    NSMutableString *ticketID = nil;
    for (TicketType *type in [GlobalConfig convertToArray:array]) {
        if ([type isKindOfClass:[TicketType class]]) {
            if ([array indexOfObject:type] == 0) {
                ticketID = [NSMutableString stringWithString:type.tTypeID];
            }
            else {
                [ticketID appendString:[NSString stringWithFormat:@",%@",type.tTypeID]];
            }
        }
    }
    return ticketID;
